...
@section('page-title', 'Persetujuan & Manajemen Cuti')
@section('page-subtitle', 'Tinjau permohonan cuti karyawan, verifikasi sisa kuota, serta lakukan persetujuan atau penolakan.')
...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div class="flex items-center gap-2 overflow-x-auto pb-1">
            ...
            <a href="{{ route('admin.leave.index', ['status' => 'rejected']) }}"
        ...
        <!-- Search Bar -->
        <form method="GET" action="{{ route('admin.leave.index') }}" class="flex items-center gap-2">
            @if($status)
            ...
            <table class="w-full text-left text-xs">
                <thead>
                    <tr class="border-b border-slate-200 text-slate-500 font-bold uppercase tracking-wider bg-slate-50/50">
                        ...
                        <th class="py-3 px-4">Durasi & Sisa Kuota</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($leaves as $item)
                        <tr class="hover:bg-slate-50/80 transition">
                            ...
                            <!-- Rentang Tanggal -->
                            <td class="py-3.5 px-4">
                                <p class="font-bold text-slate-900">{{ \Carbon\Carbon::parse($item->start_date)->format('d M Y') }}</p>
                                <p class="text-[11px] text-slate-500">s/d {{ \Carbon\Carbon::parse($item->end_date)->format('d M Y') }}</p>
                            </td>
                            ...
                            <td class="py-3.5 px-4">
                                ...
                                <p class="text-[10px] text-slate-500 mt-1">Sisa kuota: <strong class="text-slate-800">{{ $item->user->leave_quota }} hari</strong></p>
                            </td>
                            ...
                            <td class="py-3.5 px-4 text-center">
                                @if($item->status === 'pending')
                                    <div class="flex items-center justify-center gap-1.5">
                                        <!-- Approve Button -->
                                        <form action="{{ route('admin.leave.approve', $item->id) }}" method="POST" onsubmit="return confirm('Setujui cuti {{ $item->user->name }} selama {{ $item->total_days }} hari? Kuota cuti karyawan akan dipotong otomatis.')">
                                            @csrf
                                            @method('PATCH')
                                        ...
                                        <!-- Reject Button -->
                                        <button type="button" onclick="openRejectModal({{ $item->id }}, '{{ $item->user->name }}')" class="p-2 rounded-lg bg-rose-50 hover:bg-rose-100 text-rose-700 border border-rose-200 font-bold text-xs transition" title="Tolak Cuti">
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="text-center py-10 text-slate-400">
                                ...
                                <p class="text-xs font-medium">Tidak ada permohonan cuti yang ditemukan.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
